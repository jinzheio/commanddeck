...
export function useAgents(selectedProject: string | null) {
  ...
  const startAgent = async (hubUrl: string) => {
      if (!selectedProject) return { ok: false, reason: 'No project selected' };
      return window.commanddeck.startAgent({ projectName: selectedProject, hubUrl });
  };
  
  const stopAgent = async (agentId: string) => {
      return window.commanddeck.stopAgent({ agentId });
  };
